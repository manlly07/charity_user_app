...
const TableRequest = ({ requests }: TableCharitiesProps) => {
  const navigate = useNavigate()

  const columns: ColumnDef<RequestListResponse>[] = [
    {
      accessorKey: 'id',
      header: 'Request ID',
      cell: ({ row }) => <span>{row.getValue('id') ? `REQ - ${row.getValue('id')}` : '-'}</span>
    },
    {
      accessorKey: 'requestType',
      header: 'Request Type',
      cell: ({ row }) => <span className="text-sm">{row.getValue('requestType') || '-'}</span>
    },
    {
      accessorKey: 'volunteerId',
      header: 'Volunteer ID',
      cell: ({ row }) => (
        <span>{row.getValue('id') ? `VOL - ${row.getValue('volunteerId')}` : '-'}</span>
      )
    },
    {
      ...
      cell: ({ row }) => (
        <Badge
          ...
          className={cn('text-xs', {
            'bg-green-500/10 text-green-500': row.getValue('status') === 'APPROVED'.toUpperCase(),
            'bg-yellow-500/10 text-yellow-500': row.getValue('status') === 'PENDING'.toUpperCase(),
            'bg-red-500/10 text-red-500': row.getValue('status') === 'REJECTED'.toUpperCase()
          })}
        >
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate(`/admin/request/${row.original.id}`)}
          >
            <EyeOpenIcon />
          </Button>
        </div>
