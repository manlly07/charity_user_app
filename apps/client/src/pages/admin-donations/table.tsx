...
import { DonationEventListResponse } from '@/hooks/useDonationAdmin'
...
interface TableCharitiesProps {
  donations: DonationEventListResponse[]
}

const TableDonations = ({ donations }: TableCharitiesProps) => {
  const navigate = useNavigate()
  const columns: ColumnDef<DonationEventListResponse>[] = [
    {
      accessorKey: 'id',
      header: 'DonationID',
      cell: ({ row }) => <span>{row.getValue('id') ? `DON${row.getValue('id')}` : '-'}</span>
    },
    {
      accessorKey: 'title',
      header: 'Program Name',
      ...
      cell: ({ row }) => (
        <span>
          {new Intl.NumberFormat('vi-VN', {
            style: 'currency',
            currency: 'VND'
          }).format(parseFloat(row.getValue('moneyNeed') as string) || 0)}
        </span>
      )
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <Button
            variant="ghost"
            size="icon"
            onClick={() => navigate(`/admin/donations/${row.original.id}`)}
          >
            <EyeOpenIcon />
          </Button>
        </div>
